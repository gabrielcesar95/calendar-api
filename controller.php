...
switch ($case) {
    case 'add_schedule':
        $read->readFull('SELECT * FROM schedule s WHERE s.schedule_tstamp = :tstamp', "tstamp={$postData->schedule_tstamp}");
        ...
        if (!$read->getResult()) {
            ...
            //retorno msg de erro
            $json['msg'] = ['error', 'Xiii, esse horário não está disponível...'];
        }
        break;
    ...
    case 'add_appointment':
        ...
        if (empty($postData->appointment_schedule_id)) {
            $json['msg'] = ['error', 'Xiii, É necessário informar um dia e horário para a consulta'];
            break;
        }
        ...
        if (!empty($schedule) && $schedule->schedule_status == 1) {
            $json['msg'] = ['error', 'Xiii, esse horário já foi reservado.'];
            break;
        }
        ...
        $json['msg'] = ['success', 'Show, a consulta foi agendada!'];
    ...
    case 'delete_appointment':
        ...
        if(empty($appointment)){
            $json['msg'] = ['error', 'Xiii, não encontramos a consulta. Tente novamente'];
            break;
        }else{
